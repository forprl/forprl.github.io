...
### 模块配置
>每个应用模块都有独立的配置文件（位于模块目录的Conf/config.php），配置文件的定义格式支持PHP/JSON/YAML/INI/XML等方式，默认采用**PHP数组定义**，例如：刚下载的ThinkPHP3的`./Application/Home/Conf/config.php`文件内容
...
<br>类的命名格式`xxxController.class.php`,其中xxx为控制器的名字，并且首字母要大写
- 类中含有很多函数，不同的函数负责生成不同的页面等功能，如前段不同导航栏对应的内容对应不同的函数
