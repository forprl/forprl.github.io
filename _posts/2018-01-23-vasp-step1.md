...
<br>1E-4或者1E-5即可 
- EDIFFG原子收敛标准
<br>力作为收敛标准:EDIFFG为负,一般-0.01到-0.05之间 
...
### 初始结构设置
比较好的设置方法可以参照键长。比如CO在O顶位的吸附，可以参照CO2中C-O键长来设置（如增长20%）。也可以参照文献。实在不行，可以先在小体系上测试，然后再放到大体系中算。
### 弛豫参数设置
...
- 费米面附近能级电子分布的smearing是一种促进收敛的有效方法。在INCAR文件中以ISMEAR来设置。一般来说K点只有一两个的时候采用ISMEAR=0，金属体材料用ISMEAR=1或2,半导体材料用ISMEAR=-5等等。
...
# 案例
MS建模->设置输入文件->优化
## 更多案例下载[Materials Project](https://www.materialsproject.org)里面的VASP格式,看INCAR文件
## 参考
bbs.[大师兄科研网](http://www.bigbrosci.cn/LVASPTHW)
## bbs.O2单分子优化
INCAR
```
SYSTEM=O2
